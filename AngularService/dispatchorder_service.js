...
'use strict';
angular.module('myApp').factory('DispatchOrderService', ['$http', '$q','$cookies', function($http, $q,$cookies){
    ...
    function getOrdersByStore() {
        ...
        $http.get(REST_GET_FILTERED_ORDERS_URI+1+REST_SUBURI1+limit+REST_SUBURI5+3,{        
   ...
   function getFilteredOrderList(todate,formdate,itemid) {
        var tokenCookie = $cookies.get('storeToken');
        var deferred = $q.defer();
        $http.get(REST_GET_FILTERED_ORDERS_URI+1+REST_SUBURI1+limit+REST_SUBURI2+todate+REST_SUBURI3+formdate+REST_SUBURI4+itemid+REST_SUBURI5+3,{        
                    headers:{'Authorization': tokenCookie}
                  })
            .then(
            function (response) {
...
function getOrdersByPeginition(todate,formdate,pageno,itemid){
     var tokenCookie = $cookies.get('storeToken');
        var deferred = $q.defer();
        $http.get(REST_GET_FILTERED_ORDERS_URI+pageno+REST_SUBURI1+limit+REST_SUBURI2+todate+REST_SUBURI3+formdate+REST_SUBURI4+itemid+REST_SUBURI5+3,{        
                    headers:{'Authorization': tokenCookie}
                  })
            .then(
            function (response) {
