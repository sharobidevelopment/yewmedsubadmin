...
angular.module('myApp').controller('DispatchorderController', ['$scope', '$http','$cookies', '$location',  '$route', '$rootScope','DispatchOrderService', 'OrdertakingService' , '$interval' , function($scope,$http, $cookies, $location, $route, $rootScope,DispatchOrderService,OrdertakingService,$interval) {
    var self = this;
    ...
    // }, 50000) 
      ...
      function getOrderDetailsByStore(){
         DispatchOrderService.getOrdersByStore()
            .then(
                     ...
                     if($scope.orders.length == 20){
                      ...
                      for(var k=0;k<$scope.orders.length;k++){
                           ...
                           for(var l=0;l<$scope.orders[k].orderDetails.length;l++){
                               if($scope.orders[k].orderDetails[l].status == 3){
 ...
 function filterSerachResult(todate,formdate){
        DispatchOrderService.getFilteredOrderList(todate,formdate,$scope.itemid)
            .then(
                function successCallback(response) {
                    console.log(JSON.stringify(response.data));
                     $scope.orders=response.data;
...
$scope.nextData = function() {
  if($scope.formdate == '' || $scope.formdate == undefined){
    ...
    DispatchOrderService.getOrdersByPeginition($scope.formdate,$scope.todate,page,$scope.itemid)
            .then(
                function successCallback(response) {
                    console.log(response.data);
                    page= Number(page) + 1;
    ...
    function getLastOneHourOrders(){
     ...
     $scope.deliverItem = function(order,orderdetailsid) {
      var orderDetails={};
      orderDetails.id=orderdetailsid;
      console.log(orderDetails);
      //alert(orderdetailsid);
       DispatchOrderService.itemDeliver(orderDetails)
                    .then(
                        function successCallback(response) {
