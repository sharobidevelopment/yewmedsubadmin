...
angular.module('myApp').controller('EditDeliveryPersonController', ['$scope', '$http','$cookies', '$location',  '$route', '$rootScope','DeliverOrderService', 'OrdertakingService' , '$interval' , function($scope,$http, $cookies, $location, $route, $rootScope,DeliverOrderService,OrdertakingService,$interval) {
    var self = this;
    //alert("hiiiiii");
    $scope.orders=[];
    $scope.itemid=0;
    ...
    $scope.navigators = {prev:{state:true}, next:{state:true}};
     $scope.storeStatusCookie = $cookies.get('StoreStatus');
      ...
      var deliverpersonid = localStorage.getItem("deliverperonid");
      ...
      $http.get(serverurl+"/delivery/getDeliveryPersonById?id="+deliverpersonid,{                 
      headers:{'Authorization': tokenCookie}
          })
       .then(
       function (response) { 
       	   $scope.deliveryperson = response.data;
           $scope.id = response.data.id;
           for(var i=0;i<response.data.deliveryPin.length;i++){
              $( "#"+response.data.deliveryPin[i].pincode ).prop("checked", true);
           }
      ...
      function deliverypersonSubmit(){
         ...
         $scope.deliveryperson.id = $scope.id;
         ...
         for (var i = 0; i < pincodes.length; i++) {
                if (pincodes[i].checked) {
                  //var pincodeobj1 = {};
                    ...
                    pincodearr.push({'deliveryAgentId':$scope.id,'pincode':pincodes[i].value});
                }
         ...
         $http.post(serverurl+"/delivery/updateDeliveryPerson",$scope.deliveryperson,{                 
      headers:{'Authorization': tokenCookie}
          })
       ...
       function (response) {
         ...
         if(response.data.status == 0){
            ...
            //$(".deliveryperson_checkbox").prop("checked", false);
             ...
             $( ".deliveryperson_button" ).prop( "disabled", false );
             $location.path($rootScope.contextpath + "/deliverypersonlist");
        } 
       },
       function(errResponse){
