...
angular.module('myApp').controller('DeliverorderController', ['$scope', '$http','$cookies', '$location',  '$route', '$rootScope','DeliverOrderService', 'OrdertakingService' , '$interval' , function($scope,$http, $cookies, $location, $route, $rootScope,DeliverOrderService,OrdertakingService,$interval) {
    var self = this;
    ...
    // }, 50000) 
      ...
      function getOrderDetailsByStore(){
         DeliverOrderService.getOrdersByStore()
            .then(
                     ...
                     if($scope.orders.length == 20){
                      ...
                      for(var k=0;k<$scope.orders.length;k++){
                           ...
                           for(var l=0;l<$scope.orders[k].orderDetails.length;l++){
                               if($scope.orders[k].orderDetails[l].status == 4){
 ...
 function filterSerachResult(todate,formdate){
        DeliverOrderService.getFilteredOrderList(todate,formdate,$scope.itemid)
            .then(
                function successCallback(response) {
                    console.log(JSON.stringify(response.data));
                     $scope.orders=response.data;
...
$scope.nextData = function() {
  if($scope.formdate == '' || $scope.formdate == undefined){
    ...
    DeliverOrderService.getOrdersByPeginition($scope.formdate,$scope.todate,page,$scope.itemid)
            .then(
                function successCallback(response) {
                    console.log(response.data);
                    page= Number(page) + 1;
    ...
    function getLastOneHourOrders(){
     ...
     $scope.deliverItem = function(order,orderdetailsid) {
      ...
      alert(orderdetailsid);
       DeliverOrderService.itemDeliver(orderDetails)
                    .then(
                        function successCallback(response) {
